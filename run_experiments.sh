...
[[ "$algorithm_name" = 'JaccardLevenMatcher' ]] || [[ "$algorithm_name" = 'Coma' ]]
then
  parallel --will-cite --jobs "$num_jobs" python run_job.py -c ::: configuration_files/"$algorithm_name"/*/*
elif [[ "$algorithm_name" = 'Cupid' ]]
then
  for folder in configuration_files/"$algorithm_name"/*
  do
    parallel --will-cite --jobs "$num_jobs" python run_job.py -c ::: "$folder"/*
  done
